...
(function (root, plugin) {
    ...
    var Util = {
        /**
         * This is just a copy of the original Leaflet version that support a third z coo
         rdinate.
        ...
        clipSegment: function (a, b, bounds, useLastCode, round) {
            ...
            while (arg) {
                ...
                if (!(codeA | codeB)) {
                    ...
                    codeOut = codeA || codeB;
                    p = L.LineUtil._getEdgeIntersection(a, b, codeOut, bounds
                                                        , round);
    ...
    L.Hotline = L.Polyline.extend({
        ...
        options: {
            renderer: renderer({pane: 'tilePane'}),
            min: 0,
            max: 1,
            palette: {
                0.0: 'green',
        ...
        zmin: function() {
            let xyz = [...this._parts];
            xyz.push(...this._rings);
            return Math.min(... xyz.map(p => Math.min(... p.map(p => p.z))));
        },
        zmax: function() {
            let xyz = [...this._parts];
            xyz.push(...this._rings);
            return Math.max(... xyz.map(p => Math.max(... p.map(p => p.z))));
        },
        getRGBForValue: function (value) {
